#ifndef LIS3DH_ACCELEROMETER_DRIVER_H
#define LIS3DH_ACCELEROMETER_DRIVER_H

/******************** (C) COPYRIGHT 2012 STMicroelectronics ********************
* File Name          : lis3dh_acc_driver.h
* Author             : Mohammad Wajih Alam
* Author             : Abhishek Anand, Fabio Tota
* Version            : 1 Revision: 2  
* Date               : 2021-June-30
* Description        : Header file for lis3dh accelerometer
*
* 
* THIS SOFTWARE IS INSPIRED FROM ST PARTS. @ref: DocID17530 Rev 2 LIS3DH 
* reference manual
********************************************************************************/
...
// Check for memory read success or error
typedef enum {
  MEMS_SUCCESS				=		0x01,
  MEMS_ERROR				=		0x00	
} status_t;

// Enable/Disable Memory register
typedef enum {
  MEMS_ENABLE				=		0x01,
  MEMS_DISABLE				=		0x00	
} State_t;
...
// Output Data Rate (Sampling rate)/Power mode selection (page 35)
typedef enum {  
  LIS3DH_ODR_1Hz		        =		0x01,		
  ...
  LIS3DH_ODR_1600Hz_LP		        =		0x08,   //Low power mode
  LIS3DH_ODR_1344Hz_NP_5367HZ_LP        =		0x09	//HR (high-resolution) or normal mode
} LIS3DH_ODR_t;


/**@brief Enum defining possible accelerometer operating modes */ //::
typedef enum {
...
/**@brief Enum defining possible accelerometer high-pass filter modes (Table 34, page 36) */ 
typedef enum {
...
/**@brief Enum defining possible accelerometer high-pass filter cut-off frequency modes (Table 34, page 36) */ 
typedef enum {
  LIS3DH_HPFCF_0                       =               0x00,
  LIS3DH_HPFCF_1                       =               0x01,
  LIS3DH_HPFCF_2                       = 	       0x02,
...
/* Struct to store auxilary ADC with 3 separate dedicated input pins ADC1, ADC2, ADC3 */
typedef struct {
  uint16_t AUX_1;
  uint16_t AUX_2;
  uint16_t AUX_3;
...
/**@brief Enum defining possible accelerometer full-scale selection, default is 2g. (Table 38, page 37) */ 
typedef enum {
...
/**@brief Enum defining possible endian data selection. Default is LSB (Table 38, page 37) */ 
typedef enum {
  LIS3DH_BLE_LSB			=		0x00,
  LIS3DH_BLE_MSB			=		0x01
} LIS3DH_Endianess_t;

/**@brief Enum defining possible self-test selection. Default is disabled (Table 38 & 39 under CTRL_REG4, page 37) */ 
typedef enum {
...
/**@brief Enum defining FIFO modes. Default is 0x00. (Table 50 under FIFO_CTRL_REG, page 40) */
typedef enum {
...
/**@brief Enum defining Trigger selection. Default is 0x00. (Table 49 under FIFO_CTRL_REG, page 40) */
typedef enum {
  LIS3DH_TRIG_INT1                     =		0x00,
  LIS3DH_TRIG_INT2 			=		0x01
} LIS3DH_TrigInt_t;

/**@brief Enum defining SPI serial interface mode selection. Default is 0x00. (Table 38 under CTRL_REG4, page 37) */
typedef enum {
  LIS3DH_SPI_4_WIRE                    =               0x00,
  LIS3DH_SPI_3_WIRE                    =               0x01
} LIS3DH_SPIMode_t;

/**@brief Enum defining Axis enable selection. Default is 0x01. (Table 30 under CTRL_REG1 = 0x20, page 35) */
typedef enum {
...
/*Defines for inertial wake up HP filter bypassed*/
#define CTRL1     0x5F    //ctrl_reg1 - 100 hz & XYZ enable & Low Power Mode Enabled
#define CTRL2     0x09    //crrl_reg2 - HP filter bypassed
#define CTRL3     0x40    //ctrl_reg3 - IA1 interrupt on INT1
#define CTRL4     0x00    //ctrl_reg4 - Full scale selection:+-2g
#define CTRL5     0x08    //ctrl_reg5 - Latch interrupt on INT1_SRC
#define CTRL6     0x00    //ctrl_reg6 - Activity interrupt on INT2
#define THS       0x15    //INT1_THS  - Threshold set to 16mg*20 = 320mg
#define DURATION  0x0C    //INT1_DURATION - Duration set to 0.01s*12 = 0.12s 
#define CFG       0x2A    //INT1_CFG  - OR interrupt events:YH event & XH event
...
#endif // LIS3DH_ACCELEROMETER_DRIVER_H
