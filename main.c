...
void calibrazione(){  //funzione di calibrazione dell'IMU (giro, acc e magne)
	...
	for(int c = 0; c<3; c++)
        ...
        while(step<1450)
        { //36 secondi di calibrazione, muovere nello spazio
            //calib magnetometro 
...
/* 
    ...
    if(alreadywrittentoflash)
    {
        ...
        salva_calib_flash(); //se la memoria non volatile è vuota, ci salva i valori di default
*/
}
...
/*    //Accelerometer data read
    ...
    if( acc_values.x == 0 && acc_values.y == 0 && acc_values.z == 0 ) 
        nrf_gpio_pin_set(LED);
    ...
    if( gyro_values.x == 0 && gyro_values.y == 0 && gyro_values.z == 0 ) 
        nrf_gpio_pin_set(LED);
    ...
    if(!IGNORA_MAGNETOMETRO)
    {
        ...
        if( magn_values.x == 0 && magn_values.y == 0 && magn_values.z == 0 )    
            nrf_gpio_pin_set(LED);							
    }
    else 
    { //vettore magnetico dummy 
